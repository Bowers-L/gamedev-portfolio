//Edited by Logan Bowers 1/6/2022
//Couldn't get the $ symbol to work (could be a conflict with the package), so I converted all the JQuery $ stuff into the actual function calls.
//Also moved some references around to avoid TypeScript errors
...
function loadImage($targetSlide, index) {
    ...
    document.querySelector(tempImage).on('load.loadImage', () => {
        if (isSrcsetSupported && srcset) {
...
function showInitialImage(index) {
    ...
    $currentImage.on('load.currentImage', () => {
...
function openLightbox($currentGallery, targetIndex) {
    ...
    $currentGallery.find('.gallery-item').each((i, element) => {
        const $currentImageEl = document.querySelector(element).find('img');
...
function initSlides() {
    ...
    const transformSlide = (element, translateX, opacity) => {
        ...
        document.querySelector(element).off('mousemove touchmove');
        distance = 0;
    }
    ...
    const removeSlideEventListeners = () => {
        // mouse & touch event listener
        document.querySelector(currentSlideEl).off('mousedown touchstart');
        document.querySelector(currentSlideEl).off('mouseup touchend touchcancel');
    ...
    const updateLightbox = (newSlide) => {
        ...
        setTimeout(() => {
            ...
            if (newSlide == 'next') {
                ...
                loadImage(document.querySelector(prevSlideEl), index + 1);
                ...
                index = getLoopedIndex(currentIndex - 1);
                loadImage(document.querySelector(nextSlideEl), index - 1);
    ...
    const addSlideEventListeners = () => {
        ...
        // keyboard event listener
        document.querySelector(document).on('keydown.lightbox', e => {
            if (e.key == 'ArrowLeft') {
                showPrevSlide();
                updateLightbox('prev');
        ...
        // click on left arrow
        $lightbox.find('.lightbox-arrow.arrow-left').on('click', () => {
            showPrevSlide();
            updateLightbox('prev');
        });

        // click on right arrow
        $lightbox.find('.lightbox-arrow.arrow-right').on('click', () => {
            showNextSlide();
            updateLightbox('next');
        });
    }
...
function initGallery() {
        ...
        const handleIntersectionObserver = entries => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    entry.intersectionObserver.unobserve(entry.target);
                }
            });
        };
    ...
    $galleryItems.on('click', e => {
        const $currentTarget = document.querySelector(e.currentTarget);
...
function createLightbox() {
    ...
    const $lightboxWrapper = document.querySelector('<div class="lightbox-wrapper">');
    $lightbox = document.querySelector('<div class="lightbox">');

    // Header
    const $lightboxHeader = document.querySelector('<div class="lightbox-header">');
    const $lightboxNumbers = document.querySelector('<div class="lightbox-numbers"></div>');
    const $lightboxTitle = document.querySelector('<div class="lightbox-title"></div>');
    const $lightboxClose = document.querySelector('<button type="button" class="lightbox-close" aria-label="Close"></button>');
    $lightboxHeader.append($lightboxNumbers, $lightboxTitle, $lightboxClose);
    $lightbox.append($lightboxHeader);

    // Slides Wrapper
    const $slidesWrapper = document.querySelector('<div class="lightbox-slides-wrapper"></div>');
    $lightbox.append($slidesWrapper);

    // Slides
    const $prevSlide = document.querySelector('<div class="lightbox-slide" data-state="prev"></div>');
    const $currentSlide = document.querySelector('<div class="lightbox-slide" data-state="current"></div>');
    const $nextSlide = document.querySelector('<div class="lightbox-slide" data-state="next"></div>');
    $slidesWrapper.append($prevSlide, $currentSlide, $nextSlide);

    // Image
    const $lightboxImage = document.querySelector('<img class="lightbox-image" src="" alt="" draggable="false">');
    $currentSlide.append($lightboxImage);
    $prevSlide.append($lightboxImage.clone());
    $nextSlide.append($lightboxImage.clone());

    // Loading Spinner
    const $spinner = document.querySelector('<div class="spinner spinner-border" role="status"><span class="sr-only">Loading... </span></div>');
    $currentSlide.append($spinner);
    $prevSlide.append($spinner.clone());
    $nextSlide.append($spinner.clone());

    // Arrows
    const $lightboxArrowLeft = document.querySelector('<div class="lightbox-arrow arrow-left"></div>');
    const $lightboxArrowRight = document.querySelector('<div class="lightbox-arrow arrow-right"></div>');
    $lightbox.append($lightboxArrowLeft);
    $lightbox.append($lightboxArrowRight);

    // Footer
    const $lightboxFooter = document.querySelector('<div class="lightbox-footer">');
    $lightbox.append($lightboxFooter);

    // append lightbox to body
    $lightbox.appendTo($lightboxWrapper);
    $lightboxWrapper.appendTo(document.querySelector('body'));
}
